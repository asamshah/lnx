...
(function (
	...
	class NativeScrollbar {
		constructor(scrollbar) {
			...
			this.arrowNext = this.scrollbar.parentNode.querySelector(
				selectors$14.scrollbarArrowNext
			);
			this.arrowPrev = this.scrollbar.parentNode.querySelector(
				selectors$14.scrollbarArrowPrev
			);
	...
	class ProductFormReader {
		constructor(element, form, product, options) {
			...
			this.optionInputs = this._initInputs(
				selectors$12.optionInput,
				options.onOptionChange
			);

			this.planInputs = this._initInputs(
				selectors$12.planInput,
	...
	class Swapper {
		constructor(el) {
			...
			this.hovers = this.container.querySelectorAll(`[${selectors$11.hover}]`);
		...
		init() {
			this.hovers.forEach((hover) => {
				hover.addEventListener(
					"mouseenter",
					function () {
						const newContent = hover.getAttribute(selectors$11.hover);
			...
			this.hovers.forEach((hover) => {
				hover.addEventListener(
					"click",
					function () {
						const clickedContent = hover.getAttribute(selectors$11.hover);
	...
	function makeSwappers(container) {
		...
		const els = container.querySelectorAll(selectors$11.wrapper);
		els.forEach((el) => {
	...
	class RadioSwatch extends HTMLElement {
		constructor() {
			...
			this.element = this.querySelector(`[${selectors$10.swatch}]`);
			this.colorString = this.element.getAttribute(selectors$10.swatch);
			this.image = this.element.getAttribute(selectors$10.image);
			this.imageId = this.element.getAttribute(selectors$10.imageId);
			this.variant = this.element.getAttribute(selectors$10.variant);
			this.outer = this.element.closest(selectors$10.outerGrid);
			this.media = null;
		...
		replaceImage() {
			...
			if (this.imageReplace && this.imageSlide && this.imageId) {
				if (
					this.imageSlide.hasAttribute(selectors$10.dataGridImageTarget) &&
					this.imageSlide.getAttribute(selectors$10.dataGridImageTarget) !==
						this.imageId
				) {
				...
				this.imageSlide.setAttribute(
					selectors$10.dataGridImageTarget,
					this.imageId
				);
				...
				if (!this.imageSlide.hasAttribute(selectors$10.dataGridImageDefault)) {
					this.imageSlide.setAttribute(
						selectors$10.dataGridImageDefault,
	...
	class GridSwatch {
		constructor(wrap) {
			...
			const label = wrap.getAttribute(selectors$10.label).trim().toLowerCase();
		...
		init() {
			...
			this.swatches.forEach((swatch, index) => {
				...
				const image = variant.featured_media
	...
	function makeGridSwatches(container) {
		const gridSwatchWrappers = container.querySelectorAll(selectors$10.wrapper);
	...
	class Carousel extends HTMLElement {
		...
		connectedCallback() {
			...
			if (
				this.carousel &&
				this.carousel.hasAttribute(selectors$_.recentlyViewHolder)
			) {
				...
				this.carousel &&
				this.carousel.hasAttribute(selectors$_.relatedHolder)
			) {
		...
		init() {
			...
			if (this.carousel.hasAttribute(selectors$_.carouselOptions)) {
				this.customOptions = JSON.parse(
					decodeURIComponent(
						this.carousel.getAttribute(selectors$_.carouselOptions)
					)
				);
			}
		...
		initCarousel() {
			...
			this.container.addEventListener("theme:tab:change", () => {
				...
				this.carouselPrev = this.carousel.querySelector(
					selectors$_.carouselPrev
				);
				this.carouselNext = this.carousel.querySelector(
					selectors$_.carouselNext
				);
			});
			...
			if (Shopify.designMode) {
				setTimeout(() => {
					if (
						...
						!this.carousel.classList.contains(classes$C.flickityEnabled)
					) {
			...
			this.carousel.classList.toggle(
				classes$C.flickityStatic,
				this.smallItems ===
					this.carousel.querySelectorAll(selectors$_.carouselSlide).length
			);
		...
		calculatedArrowsTopPosition() {
			const carouselFirstSlidePhoto = this.container.querySelector(
				selectors$_.carouselFirstSlidePhoto
			);
		...
		visibleSlides() {
			...
			const slideWidth =
				this.carouselInstance !== null && this.carouselInstance.selectedElement
					...
					: this.carousel.querySelector(selectors$_.carouselSlide).clientWidth;
			const countSlides =
		...
		init() {
			this.sizeButtons.forEach((button) => {
				button.addEventListener("click", () => {
					...
					button
						.closest(selectors$Z.mediaHolder)
	...
	class ProductQuickView extends HTMLElement {
		...
		connectedCallback() {
			this.quickviewHolder = this.container.querySelector(
				`[${selectors$Y.quickviewHolder}]`
			);
			...
			if (this.quickviewHolder) {
				this.modalTemplate = this.quickviewHolder.querySelector(
					selectors$Y.quickviewModalTemplate
				);
				...
				this.triggerButton = this.quickviewHolder.querySelector(
					`[${selectors$Y.dataPopup}${this.modalID}]`
				);
				this.handle = this.triggerButton
					? this.triggerButton.getAttribute(
							`${selectors$Y.dataPopup}${this.modalID}`
					  )
					: null;
			...
			window
				...
				.then((response) => {
					...
					this.modalContent = this.modal.querySelector(
						selectors$Y.modalContent
					);
					this.modalContent.innerHTML =
						fresh.querySelector("[data-api-content]").innerHTML;
					this.modalScroll = this.modal.querySelector(
						selectors$Y.quickviewModalScrolls
					);
					const images = this.modalContent.querySelector(selectors$Y.mediaId);
					...
					this.modalCreate();
					this.container.classList.remove(classes$A.loading);
				});
		}
		...
		modalCreate() {
			MicroModal.show(this.modalID, {
				onShow: (modal, el, event) => {
					this.quickviewHead = modal.querySelector(selectors$Y.quickviewHead);
					this.quickviewFormArea = modal.querySelector(
						selectors$Y.quickviewFormArea
					);
					this.toggleButton = modal.querySelector(selectors$Y.toggleButton);
		...
		toggleForm(event) {
			...
			setTimeout(() => {
				...
				if (hasMediumVisiblePart || missingVisiblePart) {
					this.toggleButton.classList.remove(classes$A.hide);
					this.toggleButton.classList.add(classes$A.active);
				} else {
					this.toggleButton.classList.add(classes$A.hide);
				}
				...
				if (missingVisiblePart) {
					if (event === "loading") {
						this.quickviewFormArea.classList.add(classes$A.loading);

						setTimeout(() => {
							this.quickviewFormArea.classList.remove(classes$A.loading);
						}, 50);
					}

					this.quickviewFormArea.classList.remove(classes$A.expanded);
					this.toggleButton.classList.remove(classes$A.active);
				} else {
					this.quickviewFormArea.classList.add(classes$A.expanded);
				}
			}, 200);
		...
		clickEventToggleForm() {
			...
			this.toggleButton.addEventListener("click", () => {
				this.toggleButton.classList.toggle(classes$A.active);
				this.quickviewFormArea.classList.toggle(classes$A.expanded);
			});
		}
	...
	function memoizeGetter(obj, name) {
		...
		if (
			Reflect.getOwnPropertyDescriptor(obj.constructor.prototype, name).writable
		) {
	...
	const selectors$X = {
		...
		popdown: "[data-product-add-popdown-wrapper]", // this is the popdown that shows when you add to cart // TODO: remove this,
		drawer: '[data-drawer="drawer-cart"]', // added this so we can target the drawer
	...
	class ProductAddButton extends HTMLElement {
		...
		get drawer() {
			return document.querySelector(selectors$X.drawer);
		}
		...
		get errors() {
			const errorsEl = this.wrapper.querySelector(selectors$X.errors);

			if (!errorsEl) {
				const container = this.wrapper.closest(selectors$X.errorBoundary);
		...
		onSuccess(variant) {
			...
			if (window.theme.state.cartOpen) {
				document.dispatchEvent(
					new CustomEvent("theme:cart:reload", { bubbles: true })
				);
			} else {
				...
				document.addEventListener(
					"theme:cart:change",
					function (event) {
						console.log('cart change', event.detail.cart);
				...
				if (this.drawer) {
					this.drawer.addEventListener(
						"theme:drawer:open",
						function () {
							console.log('open cart', this.cart)
		...
		init() {
			if (this.addressNewForm) {
				...
				const newAddressFormInner = this.addressNewForm.querySelector(
					selectors$W.addressNewFormInner
				);
				this.customerAddresses();

				const newButtons = section.querySelectorAll(selectors$W.btnNew);
				...
				const editButtons = section.querySelectorAll(selectors$W.btnEdit);
				if (editButtons.length) {
					editButtons.forEach((element) => {
						element.addEventListener("click", function () {
							const formId = this.getAttribute(selectors$W.dataFormId);
							section
								.querySelector(`${selectors$W.editAddress}_${formId}`)
								.classList.toggle(selectors$W.classHide);
						});
					});
				}

				const deleteButtons = section.querySelectorAll(selectors$W.btnDelete);
				if (deleteButtons.length) {
					deleteButtons.forEach((element) => {
						element.addEventListener("click", function () {
							const formId = this.getAttribute(selectors$W.dataFormId);
							const confirmMessage = this.getAttribute(
								selectors$W.dataConfirmMessage
							);
							if (
								confirm(confirmMessage || selectors$W.defaultConfirmMessage)
							) {
								Shopify.postLink(
		...
		customerAddresses() {
			...
			if (Shopify.CountryProvinceSelector) {
				new Shopify.CountryProvinceSelector(
					selectors$W.addressCountryNew,
					selectors$W.addressProvinceNew,
					{
			...
			const countryOptions = this.section.querySelectorAll(
				selectors$W.addressCountryOption
			);
	...
	const template = document.querySelector(selectors$W.templateAddresses);
	if (template) {
	...
	const parallaxImage = {
		onLoad() {
			...
			const frames = this.container.querySelectorAll(
				selectors$S.parallaxWrapper
			);
			frames.forEach((frame) => {
				const inner = frame.querySelector(selectors$S.parallaxImg);
		...
		checkWidth() {
			...
			if (
				...
				const animationTimeFrame =
					(this.text.clientWidth / selectors$R.space) * selectors$R.moveTime;
				...
				this.text.classList.add(selectors$R.animationClass);
				let clone = this.scale.querySelector(`[${selectors$R.clone}]`);
				if (clone) {
	...
	const ticker = {
		onLoad() {
			sections$l[this.id] = [];
			const el = this.container.querySelectorAll(selectors$R.frame);
			el.forEach((el) => {
	...
	class AnnouncementSlider {
		constructor(container, el) {
			this.container = container;
			this.slideshow = el;
			const speedElement = this.slideshow.getAttribute(selectors$Q.speed);
		...
		onBlockSelect(evt) {
			...
			const slide = this.slideshow.querySelector(
				`[${selectors$Q.slideAttribute}="${evt.detail.blockId}"]`
			);

			if (!slide) return;
			const slideIndex = parseInt(
				slide.getAttribute(selectors$Q.dataSlideIndex)
			);
	...
	class CartShippingMessage {
		...
		init() {
			this.cartFreeLimitShipping =
				Number(this.cartMessage[0].getAttribute(selectors$P.limit)) *
				100 *
				this.rate;
		...
		freeShippingMessageHandle(total) {
			if (this.cartMessage.length > 0) {
				this.container
					.querySelectorAll(selectors$P.cartMessage)
					.forEach((message) => {
						const hasFreeShipping =
							message.hasAttribute(selectors$P.cartMessageValue) &&
							message.getAttribute(selectors$P.cartMessageValue) === "true" &&
		...
		cartBarProgress(progress = null) {
			this.container
				.querySelectorAll(selectors$P.cartProgress)
				.forEach((element) => {
					this.setProgress(
						element,
						progress === null
							? element.getAttribute(selectors$P.percent)
							: progress
					);
				});
		}
		...
		exchangeRateConversions(cartFreeLimitShipping, shippingAmount) {
			...
			this.container
				.querySelectorAll(selectors$P.leftToSpend)
				.forEach((element) => {
	...
	class Bar$1 {
			...
			for (let index = 0; index < this.slides.length; index++) {
				...
				if (
					this.locationPath.indexOf(
						element.getAttribute(selectors$O.dataTargetReferrer)
					) === -1 &&
		...
		initTickers(stopClone = false) {
			const frame = this.barHolder.querySelector(selectors$O.frame);
		...
		toggleTicker(e, isStoped) {
			...
			const element = document.querySelector(
				`[${selectors$O.slideValue}="${e.detail.blockId}"]`
			);

			if (isStoped && element) {
				tickerScale.setAttribute("data-stop", "");
				tickerScale
					.querySelectorAll(selectors$O.tickerText)
					.forEach((textHolder) => {
		...
		onBlockSelect(e) {
			if (this.slider && typeof this.slider.onBlockSelect === "function") {
				...
				const slides = document.querySelectorAll(
					`[${selectors$O.slideAttribute}="${e.detail.blockId}"]`
				);

				slides.forEach((slide) => {
					if (slide.classList.contains(classes$x.mobileClass)) {
						this.hasDeviceClass = classes$x.mobileClass;
					}

					if (slide.classList.contains(classes$x.desktopClass)) {
						this.hasDeviceClass = classes$x.desktopClass;
					}
	...
	class Bar {
		 ...
		 */
		initTickers(stopClone = false) {
			const frame = this.container.querySelector(selectors$N.frame);
		...
		toggleTicker(e, isStoped) {
			const tickerScale = this.container.querySelector(selectors$N.tickerScale);
			const element = this.container.querySelector(
				`[${selectors$N.slideValue}="${e.detail.blockId}"]`
			);

			if (isStoped && element && tickerScale) {
				tickerScale.setAttribute("data-stop", "");
				tickerScale
					.querySelectorAll(selectors$N.tickerText)
					.forEach((textHolder) => {
	...
	class Drawer {
		...
		staggerChildAnimations() {
			this.staggers.forEach((el) => {
				const children = el.querySelectorAll(selectors$M.children);
		...
		showDrawer() {
			...
			setTimeout(() => {
				...
				this.drawer.classList.add(classes$v.isVisible);
				document.dispatchEvent(
					new CustomEvent("theme:scroll:lock", {
						bubbles: true,
				...
				const firstFocus = this.drawer.querySelector(selectors$M.focusable);
		...
		hideDrawer() {
			...
			document.dispatchEvent(
				new CustomEvent("theme:sliderule:close", { bubbles: false })
			);
			removeTrapFocus();
			...
			setTimeout(() => {
				if (!this.drawer.classList.contains(classes$v.isVisible)) {
					this.drawer.classList.add(classes$v.displayNone);
				}
			}, 800);
		}
	}
	...
	class Sticky {
		constructor(el) {
			...
			const announcementEl = document.querySelector(selectors$L.announcement);
	...
	class HoverDisclosure {
		constructor(el) {
			...
			const btnSelector = `[${selectors$K.disclosureToggle}='${this.key}']`;
		...
		staggerChildAnimations() {
			const simple = this.disclosure.querySelectorAll(selectors$K.stagger);
			simple.forEach((el, index) => {
				el.style.transitionDelay = `${index * 50 + 10}ms`;
			});

			const pairs = this.disclosure.querySelectorAll(selectors$K.staggerPair);
			pairs.forEach((child, i) => {
				const d1 = i * 150;
				child.style.transitionDelay = `${d1}ms`;
				child.parentElement
					.querySelectorAll(selectors$K.staggerAfter)
			...
			const images = this.disclosure.querySelectorAll(selectors$K.staggerImage);
	...
	const hoverDisclosure = {
		onLoad() {
			...
			disclosures = this.container.querySelectorAll(
				selectors$K.disclosureWrappper
			);
	...
	class HoverLine {
		constructor(el) {
			...
			this.underlineCurrent =
				this.sectionOuter.getAttribute(selectors$J.underlineCurrent) === "true";
		...
		setHeight() {
			...
			const text = this.itemList[0].querySelector(selectors$J.text);
		...
		listen() {
			this.itemList.forEach((element) => {
				element.addEventListener("mouseenter", (evt) => {
					const item = evt.target.querySelector(selectors$J.text);
					this.startBar(item);
				});
			});
		...
		startBar(item) {
			this.setHeight();
			let active =
				this.sectionOuter.getAttribute(selectors$J.isActive) !== "false";
	...
	class Totals {
		constructor(el) {
			this.section = el;
			this.counts = this.section.querySelectorAll(`[${selectors$I.count}]`);
			this.prices = this.section.querySelectorAll(`[${selectors$I.price}]`);
			this.dots = this.section.querySelectorAll(`[${selectors$I.dot}]`);
			this.cart = null;
			this.listen();
		}
		...
		update() {
			if (this.cart) {
				this.prices.forEach((price) => {
					price.setAttribute(selectors$I.price, this.cart.total_price);
				...
				this.counts.forEach((count) => {
					count.setAttribute(selectors$I.count, this.cart.item_count);
	...
	class SearchPopdownTriggers {
		...
		hidePopdown() {
			...
			this.input.dispatchEvent(
				new CustomEvent("theme:search:clear", { bubbles: false })
			);
			this.popdown.dispatchEvent(
				new CustomEvent("theme:scroll:unlock", { bubbles: true })
			);
	...
	const searchPopdown = {
		onLoad() {
			sections$e[this.id] = {};
			// Adjusting the selector to account for both mobile and desktop versions
			const triggers = this.container.querySelectorAll(
				`[${selectors$H.popdownTrigger}="search-popdown"], [${selectors$H.popdownTrigger}="search-popdown-desktop"]`
			);

			triggers.forEach(trigger => {
				sections$e[this.id][trigger.getAttribute(selectors$H.popdownTrigger)] = new SearchPopdownTriggers(trigger);
			});
		},
		onUnload: function () {
			for (let key in sections$e[this.id]) {
				if (typeof sections$e[this.id][key].unload === "function") {
					sections$e[this.id][key].unload();
				}
			}
		},
	};
	...
	class ProductGridItem extends HTMLElement {
		constructor() {
			...
			this.gridItemVariantLinks = this.container.querySelectorAll(
				`a[${selectors$G.dataGridItemVariant}]`
			);
		}
		...
		siblingHoverEvent(element) {
			element.addEventListener("mouseenter", () => {
				if (element.hasAttribute(selectors$G.dataGridItemVariant)) {
					...
					const product = element.getAttribute(selectors$G.dataGridItemVariant);

					this.container
						.querySelectorAll(
							`product-grid-item-content[${selectors$G.dataGridItemVariant}]`
						)
						.forEach((siblingElement) => {
							if (
								siblingElement.getAttribute(selectors$G.dataGridItemVariant) ===
								product
							) {
								siblingElement
									.querySelectorAll(`[${selectors$G.dataGridImage}]`)
									.forEach((image) => {
		...
		siblingClickEvent(element) {
			element.addEventListener("click", (e) => {
				e.preventDefault();
				const siblingActive = this.container.querySelector(
					`a[${selectors$G.dataGridItemVariant}][aria-current="true"]`
				);
	...
	class ProductGridItemContent extends HTMLElement {
		static get observedAttributes() {
			return ["hidden"];
		}
		...
		constructor() {
			...
			this.mobileStopSlideshow =
				this.container.getAttribute(selectors$F.dataStopSlideshowMobile) ===
				"true";
			this.imagesHolder = this.container.querySelector(
				`[${selectors$F.dataGridImages}]`
			);
			this.images = this.container.querySelectorAll(
				`[${selectors$F.dataGridImage}]`
			);
			this.links = this.container.querySelectorAll(
				selectors$F.productGridItemLink
			);
			this.imagesLimit =
				this.imagesHolder &&
				this.imagesHolder.hasAttribute(selectors$F.dataGridImages)
					? parseInt(this.imagesHolder.getAttribute(selectors$F.dataGridImages))
					: 5;
			this.timeout = times$1.timeout;
			this.interval = null;
		...
		connectedCallback() {
			if (this.images.length > 1) {
				...
				this.container.addEventListener("theme:swatch:change", (e) => {
					...
					const target = this.container.querySelector(
						`[${selectors$F.dataGridImageTarget}="${id}"]`
					);
					if (target) {
						const index = target.getAttribute(selectors$F.dataGridImage);
			...
			if (this.images.length > 1) {
				...
				this.container
				.querySelectorAll(`[${selectors$F.dataGridImage}]`)
				.forEach((image) => {
					image.classList.add("lazyload", "lazypreload");
				});

				let imageCount = this.container.images.length
				this.container.images.forEach((image, index) => {
					let touchstartX = 0
					let touchendX = 0
					...
					image.addEventListener('touchend', e => {
					  ...
					  if (touchendX < touchstartX) {
						// swiped right, go forward an image
				...
				this.container.addEventListener("mouseenter", () => {
					...
					// Added this to skip directly to the second image
					this.pagingProgressCounter = 100;
					this.progressPaging();
				});
				...
				this.container.addEventListener("mouseleave", () => {
					...
					this.changeImage(0); // Reset to the first image
					this.progressPaging();
				  });
			}
		}
		...
		changeToCurrentVariantImage() {
			const currentVariantImage = this.container.querySelector(
				`[${selectors$F.dataGridImageCurrentVariant}]`
			);

			if (!currentVariantImage) return;

			const imageIndex = currentVariantImage.getAttribute(
				selectors$F.dataGridImage
			);
		...
		changeImage(index = null) {
			...
			const currentImage = this.container.querySelector(
				`[${selectors$F.dataGridImage}].${classes$r.active}`
			);
			const changedImage =
				index !== null
					? this.images[index]
			...
			if (changedImage) {
				const changedImageIdx = changedImage.hasAttribute(
					selectors$F.dataGridImage
				)
					? changedImage.getAttribute(selectors$F.dataGridImage)
					: 0;
		...
		changePaging(index = 0) {
			...
			const activePage = this.container.querySelector(
				`[${selectors$F.dataGridPage}].${classes$r.active}`
			);
			const currentPage = this.container.querySelector(
				`[${selectors$F.dataGridPage}="${index}"]`
			);
			...
			if (this.imagesHolder) {
				...
				if (pagination !== "") {
					let pagingContainer = this.container.querySelector(
						`[${selectors$F.dataGridPagination}]`
					);
					if (!pagingContainer) {
	...
	class PopoutSelect extends HTMLElement {
		constructor() {
			...
			if (
				this.popoutInput &&
				this.popoutInput.hasAttribute(selectors$E.popoutQuantity)
			) {
				document.addEventListener(
					"theme:popout:update",
		...
		popupToggleClick(evt) {
			const ariaExpanded =
				evt.currentTarget.getAttribute(selectors$E.ariaExpanded) === "true";
			...
			if (link.attributes.href.value === "#") {
				...
				if (this.popoutPrevent) {
					...
					const targetAttribute = this.popoutList.querySelector(
						`[${selectors$E.ariaCurrent}]`
					);

					if (
						targetAttribute &&
						targetAttribute.hasAttribute(`${selectors$E.ariaCurrent}`)
					) {
		...
		updatePopout(evt) {
			const targetElement = this.popoutList.querySelector(
				`[${selectors$E.dataValue}="${this.popoutInput.value}"]`
			);
			if (targetElement) {
	...
	class ShippingCalculator {
		...
		init() {
			...
			let locale = "en";
			if (
				htmlEl.hasAttribute(selectors$C.lang) &&
				htmlEl.getAttribute(selectors$C.lang) !== ""
			) {
			...
			if (
				this.country &&
				this.country.hasAttribute(selectors$C.defaultData) &&
				this.province &&
				this.province.hasAttribute(selectors$C.defaultData)
			) {
	...
	class CartItems {
		constructor(section) {
			...
			this.cartMessages = document.querySelectorAll(
				selectors$A.cartMessageContainer
			);
		...
		lockState() {
			this.latestClick.querySelector(selectors$A.itemLoadbar).style.display =
				"block";
	...
	class Accordion {
		constructor(el) {
			...
			this.recentlyClicked = false;  // Flag to check if button was recently clicked
			this.trigger.addEventListener("click", (e) => this.clickHandler(e));
		...
		clickHandler(e) {//New Method for handling double
			console.log("Click handler called");
			e.preventDefault();

			if (this.recentlyClicked) {
				return;  // If button was recently clicked, do nothing
			}

			this.clickEvents(e);  // Execute accordion open/close logic

			this.recentlyClicked = true;  // Set flag to true to prevent subsequent rapid clicks

			setTimeout(() => {
				this.recentlyClicked = false;  // Reset flag after debounce time
			}, 600);
		}
		...
		getSiblings() {
			...
			const groupsArray = [
				...section.querySelectorAll(selectors$z.accordionGroup),
			];
			...
			if (syncWrapper) {
				const allChilden = [
					...syncWrapper.querySelectorAll(selectors$z.accordionBody),
				];
		...
		closeSiblings() {
			this.syncBodies.forEach((accordionBody) => {
				accordionBody.dispatchEvent(
					new CustomEvent("theme:accordion:close", { bubbles: false })
				);
			});
		}
		...
		toggleState() {
			if (this.trigger.classList.contains(classes$n.open)) {
				...
				if (this.body.hasAttribute(selectors$z.rangeSlider)) {
					setTimeout(() => {
						document.dispatchEvent(
							new CustomEvent("theme:price-range:reset", { bubbles: false })
						);
					}, 400);
				}
			}
	...
	const accordion = {
		...
		onSelect: function () {
			if (this.type === "accordion-single") {
				this.container
					.querySelector(`[${selectors$z.accordionToggle}]`)
					.click();
			}
		},
		onDeselect: function () {
	...
	class SearchPredictive {
		...
		render(terms) {
			...
			const serialized = `${window.theme.routes.root_url}search/suggest.json?q=${terms}&resources[type]=${resources}&resources[options][unavailable_products]=last`;
			fetch(serialized)
				.then(this.handleErrors)
		...
		injectProduct(products) {
			...
			products.forEach((p) => {
				...
				if (product.featured_image) {
					product.thumb = getSizedImageUrl(
	...
	class HeaderMobileSliderule {
		...
		hideSliderule(close = false) {
			...
			const children = close
				? this.pane.querySelectorAll(`.${classes$l.isVisible}`)
				: this.children;
			...
			const hidedSelector = close
				? `[${selectors$v.dataAnimates}].${classes$l.isHidden}`
				: `[${selectors$v.dataAnimates}="${newPosition}"].${classes$l.isHidden}`;
	...
	class CartPopdown {
		constructor() {
			...
			this.cartSectionAjax = document.querySelector(
				selectors$u.cartSectionAjax
			);
		...
		renderPopdown(event) {
			...
			axios
				...
				.then(function (response) {
					...
					instance.drawer.innerHTML = fresh.querySelector(
						selectors$u.apiContent
					).innerHTML;
		...
		connectCartButton() {
			...
			const cartButton = this.drawer.querySelector(
				selectors$u.cartToggleButton
			);
	...
	class Header {
		constructor(el) {
			...
			// this.transparent =
			// 	this.wrapper.getAttribute(selectors$t.transparent) !== "false";
	...
	function embedYoutube(uniqueKey, options) {
		...
		const youtubeKey = playerWrapper
			.querySelector(`[${selectors$r.videoId}]`)
		...
		init() {
			this.triggers.forEach((trigger) => {
				...
				const player = document.querySelector(
					`[${selectors$p.attrPlayer}="${uniqueKey}"]`
				);
	...
	class ScrollButton {
		...
		scroll() {
			const bottom = this.wrapper.offsetTop + this.wrapper.clientHeight - document.querySelector('.site-header').offsetHeight;
			window.scroll({
				top: bottom,
				left: 0,
				behavior: "smooth",
	...
	class Slideshow {
		constructor(section) {
			...
			this.autoplay =
				this.wrapper.getAttribute(selectors$m.autoplay) === "true";
			this.slideCount = parseInt(
				this.wrapper.getAttribute(selectors$m.slideCount),
				10
			);
		...
		unload() {
			...
			if (
				this.flkty &&
				this.wrapper &&
				this.wrapper.classList.contains(classes$g.isEnable)
			) {
				this.flkty.destroy();
			}
		}
		...
		setDefaultValues() {
			...
			if (this.slider.hasAttribute(selectors$l.dataMinValue)) {
				defaultMinValue = parseFloat(
					this.slider.getAttribute(selectors$l.dataMinValue)
				);
			}
			...
			if (this.slider.hasAttribute(selectors$l.dataMaxValue)) {
				defaultMaxValue = parseFloat(
					this.slider.getAttribute(selectors$l.dataMaxValue)
				);
			}
			...
			if (this.slider.getAttribute(selectors$l.dataStep)) {
				this.step = Math.abs(
					parseFloat(this.slider.getAttribute(selectors$l.dataStep))
				);
			}
		...
		onChanged(min, max) {
			if (this.slider.hasAttribute(selectors$l.dataFilterUpdate)) {
				this.slider.dispatchEvent(
					new CustomEvent("theme:range:update", { bubbles: true })
				);
			}
		}
	...
	class Filters {
		constructor(filters, refresh = false) {
			...
			if (this.getShowOnLoad() || (shouldOpenFilters)) {
				this.showFilters();
			} else {
				this.hideFilters();
			}
			...
			if (this.showMoreButtons.length) {
				this.showMoreButtons.forEach((button) => {
					button.addEventListener(
						"click",
						throttle(this.connectShowHiddenOptions, 500)
					);
				});
			}
		}
		...
		showHiddenOptions(evt) {
			const element = evt.target.hasAttribute(selectors$k.showMore)
				? evt.target
				: evt.target.closest(`[${selectors$k.showMore}]`);

			element.classList.add(classes$e.hidden);

			element.previousElementSibling
				.querySelectorAll(`.${classes$e.hidden}`)
				.forEach((option) => {
		...
		connectToggle(refresh = false) {
			this.filtersToggleButtons.forEach((button) => {
				const isDynamic =
					button.closest(`[${selectors$k.filtersWrappper}]`) !== null;
		...
		showFilters() {
			...
			setTimeout(() => {
				...
				this.filtersToggleButtons.forEach((btn) =>
					btn.classList.add(classes$e.show)
				);
		...
		hideFilters() {
			...
			this.filtersToggleButtons.forEach((btn) =>
				btn.classList.remove(classes$e.show)
			);
			this.filtersToggleButtons.forEach((btn) =>
				btn.classList.remove(classes$e.defaultVisible)
			);
			// adds display none after animations
			setTimeout(() => {
	...
	class FiltersForm {
			...
			if (this.form) {
				...
				this.sidebar.addEventListener(
					...
					debounce((e) => {
						...
						if (
							!selectors$j.inputs.includes(type) ||
							!this.form ||
						...
						const priceMin = this.form.querySelector(
							`[${selectors$j.priceMin}]`
						);
						const priceMax = this.form.querySelector(
							`[${selectors$j.priceMax}]`
						);

						if (
							target.hasAttribute(selectors$j.priceMin) ||
							target.hasAttribute(selectors$j.priceMax)
						) {
							const rangeMin = this.form.querySelector(
								`[${selectors$j.rangeMin}]`
							);
							const rangeMax = this.form.querySelector(
								`[${selectors$j.rangeMax}]`
							);
							parseInt(rangeMin.getAttribute(selectors$j.rangeMinDefault));
							const rangeMaxDefault = parseInt(
								rangeMax.getAttribute(selectors$j.rangeMaxDefault)
							);
        ...
        return true;
			...
			// sets to false if on search page > 2, otherwise true
			let checkSearchPage = !isSearchPage();

			// Call this function when the page loads
			const initializeFilters = () => {
				const inStockCheckbox = this.form.querySelector('input[name="filter.v.availability"][value="1"]');
				if (checkSearchPage && inStockCheckbox && !inStockCheckbox.checked) {
					...
					// Set firstLoad to false so this logic doesn't run again and allows for users to change In Stock
					firstLoad = false;
				}
			};

			// Makes In Stock cancel Out of Stock and vice versa
			const handleFilterChange = (e) => {

				const inStockCheckbox = this.form.querySelector('input[name="filter.v.availability"][value="1"]');
				const outOfStockCheckbox = this.form.querySelector('input[name="filter.v.availability"][value="0"]');

				if (e.target === inStockCheckbox && inStockCheckbox.checked) {
					outOfStockCheckbox.checked = false;
				} else if (e.target === outOfStockCheckbox && outOfStockCheckbox.checked) {
					inStockCheckbox.checked = false;
				}
			};

			// Modify the event listener for filter changes to call handleFilterChange
			if (this.sidebar) {
				this.sidebar.addEventListener("input", handleFilterChange);
			}

			// Call initializeFilters to check the "In Stock" checkbox on first load
			initializeFilters();
		...
		updateRange(e) {
			if (this.form && typeof this.form.submit === "function") {
				...
				if (
					checkElements &&
					rangeMin.hasAttribute(selectors$j.rangeMin) &&
					rangeMax.hasAttribute(selectors$j.rangeMax)
				) {
					...
					const rangeMinValue = parseInt(
						rangeMin.getAttribute(selectors$j.rangeMin)
					);
					const rangeMaxValue = parseInt(
						rangeMax.getAttribute(selectors$j.rangeMax)
					);
		...
		updateFilterFromUrl(e) {
			// This fires when an accordion is clicked to open or twice when a filter is clicked
			const target = e.target
			// This if statement checks if this is a button that removes a filter
			if (
				target.matches(selectors$j.filterUpdateUrlButton) ||
				(target.closest(selectors$j.filterUpdateUrlButton) && target)
			) {
				e.preventDefault();
				const button = target.matches(selectors$j.filterUpdateUrlButton)
					? target
					: target.closest(selectors$j.filterUpdateUrlButton);
		...
		addToHistory(e, replaceHref = "") {
			...
			const sortValue = this.sort
				? this.sort.getAttribute(selectors$j.dataSort)
				: "";
			...
			if (!e || (e && e.type !== "popstate")) {
				if (replaceHref === "") {
					...
					if (this.form) {
						...
						if (
							checkElements &&
							rangeMin.hasAttribute(selectors$j.rangeMinDefault) &&
							rangeMax.hasAttribute(selectors$j.rangeMaxDefault)
						) {
							const rangeMinDefault = parseFloat(
								rangeMin.getAttribute(selectors$j.rangeMinDefault),
								10
							);
					...
					// Add a search param for availability if it doesn't exist, regardless of the filter
					if (searchParams.has("filter.v.availability")) {
						// console.log("Has Availability filter");
					} else {
		...
		getFilterResult() {
			...
			this.sidebar.classList.add(classes$d.classLoading);
			this.getGridValues();
			...
			fetch(`${window.location.pathname}${window.location.search}`)
				...
				.then((response) => {
					...
					setTimeout(() => {
						this.sidebar.classList.remove(classes$d.classLoading);
		...
		refreshFunctionalities() {
			...
			__Theme.unmountComponents();
			__Theme.mountComponents();
		}
		...
		showAllOptions() {
			...
			checkedOptions.forEach((option) => {
				if (
					...
					option
						.closest(selectors$j.optionHolder)
						.classList.contains(classes$d.classHidden)
				) {
		...
		filtering(e, replaceHref = "") {
			...
			const headerH = this.headerIsSticky
				? document
						.querySelector(selectors$j.headerHeight)
						.getBoundingClientRect().height
				: 0;
			// Changed this from the collectionTools to the activefiltersholder because we've fixed the position of the filters
			const collectionTools = this.container.querySelector(
				selectors$j.activeFiltersHolder
			);
			const scrollToElement = collectionTools.offsetTop - headerH;
			const enableScrollTo = windowScrollY > headerH;
	...
	function initSectionModels(modelViewerContainer, sectionId) {
		...
		if (modelViewerContainer.closest(selectors$h.productSlideshow) !== null) {
			const xrButton = modelViewerContainer
				.closest(selectors$h.productSlideshow)
				.parentElement.querySelector(selectors$h.productXr);
		...
		for (const sectionId in modelJsonSections) {
			if (modelJsonSections.hasOwnProperty(sectionId)) {
				...
				const modelJson = document.querySelector(
					`${selectors$h.modelJson}${sectionId}`
				);
				if (modelJson) {
	...
	class Media {
		constructor(section) {
			...
			this.slideshow = this.container.querySelector(
				selectors$g.productSlideshow
			);
			this.mobileStyle = this.slideshow.getAttribute(
				selectors$g.slideshowMobileStyle
			);
			this.desktopStyle = this.slideshow.getAttribute(
				selectors$g.slideshowDesktopStyle
			);
			...
			this.leftThumbsHolder = this.container.querySelector(
				selectors$g.leftThumbsHolder
			);
			this.thumbWrapper = this.container.querySelector(
				selectors$g.productThumbs
			);
			this.thumbImages = this.container.querySelectorAll(
				selectors$g.thumbImage
			);
			this.loopVideo =
				this.container.getAttribute(selectors$g.loopVideo) === "true";
			this.verticalAlignment = Boolean(
				this.container.querySelector(selectors$g.verticalAlignment)
			);
		...
		doFlktyChange(index) {
			...
			const currentThumb = this.thumbWrapper?.querySelector(
				`[${selectors$g.dataMediaSelect}="${id}"]`
			);

			this.thumbWrapper
				?.querySelector(`.${classes$c.active}`)
				.classList.remove(classes$c.active);
		...
		detect3d() {
			...
			if (modelViewerElements) {
				...
				document.addEventListener(
					...
					function () {
						this.container
							.querySelectorAll(selectors$g.allPlayers)
							.forEach((player) => {
		...
		detectYouTube() {
			const playerElements = this.section.container.querySelectorAll(
				selectors$g.videoPlayerExternal
			);
			for (var player of playerElements) {
				const uniqueKey = player.dataset.player;
		...
		pauseOtherMedia(uniqueKey) {
			const otherMedia = this.container.querySelector(
				`${selectors$g.mediaSlide}:not([data-player="${uniqueKey}"])`
			);
		...
		scrollThumbs(event) {
			...
			if (this.thumbs) {
				...
				this.thumbsHeight =
					...
					this.thumb.getAttribute(selectors$g.aspectRatio);
	...
	class PickupAvailability {
		...
		fetchPickupAvailability(event) {
			const container = this.container.querySelector(
				`[${selectors$f.pickupContainer}]`
			);
			if (!container) return;
			const variantID =
				event && event.detail.variant
					? event.detail.variant.id
					: container.getAttribute(selectors$f.pickupContainer);
				...
				fetch(
					...
					.then((text) => {
						const pickupAvailabilityHTML = new DOMParser()
							...
							.querySelector(selectors$f.shopifySection).innerHTML;
		...
		update() {
			...
			this.optionElements.forEach((el) => {
				...
				const positionString = el
					.closest(`[${selectors$e.optionPosition}]`)
					.getAttribute(selectors$e.optionPosition);
	...
	class ProductForm extends HTMLElement {
		...
		updateAddToCartState(formState) {
			...
			const buttonPriceText = buttonsWrapper.querySelectorAll(selectors$d.priceButton);
			...
			// Update the button's price text
			if (buttonPriceText.length) {
				buttonPriceText.forEach((element) => {
					if (variant && variant.available) {
						console.log('Setting of button text:', variant.title);
			...
			if (
				...
				if (
					...
					this.remainingWrapper.classList.remove(
						classes$9.remainingIn,
						classes$9.remainingOut,
						classes$9.remainingUnavailable
					);
		...
		updateProductUnits(formState) {
			...
			if (unitPrice) {
				...
				this.outerWrapper.querySelector(selectors$d.unitPrice).innerHTML =
				...
				triggers.forEach((trigger) => {
					trigger.addEventListener("click", (event) => {
						...
						const dataId = event.target
							.closest(`[${selectors$c.mediaId}]`)
							.getAttribute(selectors$c.mediaId)
							.toString();
						const items = [];
						...
						const options = {
							...
							getThumbBoundsFn: function getThumbBoundsFn() {
								let imageLocation = container.querySelector(
									selectors$c.slideshow
								);
								if (!imageLocation) {
									imageLocation = container.querySelector(
										selectors$c.singeImage
									);
								}
								const pageYScroll =
									window.pageYOffset || document.documentElement.scrollTop;
	...
	class GlobalTabs {
		...
		tabChange(element, tab) {
			this.container
				.querySelector(`${selectors$b.tabsLi}.${classes$8.classCurrent}`)
				.classList.remove(classes$8.classCurrent);
			this.container
				.querySelector(`${selectors$b.tabContent}.${classes$8.classCurrent}`)
				.classList.remove(classes$8.classCurrent);
		...
		checkVisibleTabLinks() {
			...
			const tabsNavListHided = this.container.querySelectorAll(
				`${selectors$b.tabLink}.${classes$8.classHide}`
			);
		...
		checkRecentTab() {
			...
			if (tabLink) {
				...
				const tabContent = this.container.querySelector(
					`${selectors$b.tabContent}[${selectors$b.dataTabIndex}="${tabLinkIdx}"]`
				);
		...
		hideRelatedTab() {
			...
			const parentTabContent = relatedSection.closest(
				`${selectors$b.tabContent}.${classes$8.classCurrent}`
			);
			if (!parentTabContent) {
				return;
			}
			const parentTabContentIdx = parseInt(
				parentTabContent.getAttribute(selectors$b.dataTabIndex)
			);
			const tabsNavList = this.container.querySelectorAll(selectors$b.tabsLi);
		...
		onBlockSelect(evt) {
			const element = this.container.querySelector(
				`${selectors$b.tabLink}[${selectors$b.blockId}="${evt.detail.blockId}"]`
			);
			if (element) {
	...
	class GroupVariantImages {
		...
		showImages() {
			this.thumbs.forEach((thumb) => {
				if (
					thumb.getAttribute(selectors$9.groupImage) === "" ||
					thumb.getAttribute(selectors$9.groupImage) === this.variantImageAlt
				) {
			...
			this.slides.forEach((slide) => {
				if (
					slide.getAttribute(selectors$9.groupImage) === "" ||
					slide.getAttribute(selectors$9.groupImage) === this.variantImageAlt
				) {
	...
	class ProductTemplate {
		constructor(section) {
			...
			this.truncateElementHolder = this.container.querySelector(
				selectors$8.toggleTruncateHolder
			);
			this.truncateElement = this.container.querySelector(
				`[${selectors$8.toggleTruncateContent}]`
			);
		...
		checkExpandedText() {
			const readLessAction = this.truncateElementHolder.querySelector('.read-less-action');
			if (this.truncateElementHolder.classList.contains(classes$5.classVisible)) {
				readLessAction.style.display = "block";
			} else {
				readLessAction.style.display = "none";
			}
		}

		init() {
			this.zoomEnabled =
				this.container.querySelector(selectors$8.zoomButton) !== null;
			if (this.zoomEnabled) {
		...
		truncateText() {
			const readMoreAction = this.truncateElementHolder.querySelector('.read-more-action');
			const readLessAction = this.truncateElementHolder.querySelector('.read-less-action');
			...
			const truncateLines = this.truncateElementHolder.getAttribute('data-truncate-lines') || 5;  // Default to 5 if not found

			ellipsis(truncateAppendedElement, parseInt(truncateLines, 10), {
				replaceStr: "",
				delimiter: " ",
			});
			...
			if (this.truncateElement.innerHTML !== truncateAppendedElement.innerHTML) {
				...
				readMoreAction.style.display = "block";  // Show Read More
			} else {
				truncateAppendedElement.remove();
				this.truncateElementHolder.classList.remove(classes$5.classExpanded);
				readMoreAction.style.display = "none";  // Hide Read More
			}
		...
		toggleTruncatedContent(holder) {
			const toggleButtonMore = holder.querySelector('.read-more-action ' + selectors$8.toggleTruncateButton);
			const toggleButtonLess = holder.querySelector('.read-less-action ' + selectors$8.toggleTruncateButton);

			if (toggleButtonMore) {
				toggleButtonMore.addEventListener("click", (e) => {
					e.preventDefault();
					holder.classList.add(classes$5.classVisible);
					holder.classList.remove(classes$5.classExpanded);
					toggleButtonMore.parentElement.style.display = "none";
					toggleButtonLess.parentElement.style.display = "block";
				});
			}

			if (toggleButtonLess) {
				toggleButtonLess.addEventListener("click", (e) => {
					e.preventDefault();
					holder.classList.remove(classes$5.classVisible);
					holder.classList.add(classes$5.classExpanded);
					toggleButtonLess.parentElement.style.display = "none";
					toggleButtonMore.parentElement.style.display = "block";
				});
			}
		}
	...
	class Toggle {
		...
		toggleEvent(evt = true) {
			...
			const activeToggle = this.toggle.parentElement.querySelector(
				`[${selectors$7.toggle}].${classes$4.active}`
			);
			let currentToggle = this.toggle.parentElement.querySelector(
				`[${selectors$7.toggle}="${gridNumber}"]`
			);
	...
	class Sort {
		...
		sortingResults(e) {
			...
			this.sort
				.querySelector(`.${classes$3.active}`)
				.classList.remove(classes$3.active);
	...
	class Collection {
		...
		removeUnusableFilters() {
			...
			const collectionNavGrouped = this.container.querySelectorAll(
				selectors$5.collectionNavGrouped
			);
	...
	styles.light = [
		{
			...
			elementType: "labels",
			stylers: [{ visibility: "on" }, { lightness: "64" }, { hue: "#ff0000" }],
		},
		{
			featureType: "administrative",
	...
	styles.light_blank = [
		...
		},
		{
			featureType: "administrative",
			elementType: "labels",
			stylers: [{ visibility: "off" }, { lightness: "64" }, { hue: "#ff0000" }],
		},
		{
			featureType: "administrative",
			elementType: "labels.text.fill",
		...
		},
		{
			featureType: "road.local",
			elementType: "labels",
			stylers: [{ visibility: "off" }, { weight: "0.01" }, { lightness: "39" }],
		},
		{
			featureType: "road.local",
			elementType: "labels.text.stroke",
	...
	class Related {
		...
		init() {
			const relatedSection = this.container.querySelector(
				selectors$3.dataRelatedSectionElem
			);
			...
			const layoutLarge = relatedSection.getAttribute(
				selectors$3.dataLargeLayout
			);
			const layoutMedium = relatedSection.getAttribute(
				selectors$3.dataMediumLayout
			);
			const layoutSmall = relatedSection.getAttribute(
				selectors$3.dataSmallLayout
			);
			const route =
			...
			axios
				...
				.then((response) => {
					...
					if (inner.querySelector(selectors$3.dataRelatedProduct)) {
						...
						relatedSection
							.querySelector(selectors$3.carousel)
						...
						relatedSection.dispatchEvent(
							new CustomEvent("theme:related-products:added", { bubbles: true })
						);
					} else {
						relatedSection.dispatchEvent(
		...
		recent() {
			const recentlyViewedHolder = this.container.querySelector(
				selectors$3.recentlyViewed
			);
			const recentlyViewedWrapper = this.container.querySelector(
				selectors$3.recentlyViewedWrapper
			);
			const recentProducts = this.container.querySelectorAll(
				selectors$3.dataRelatedProduct
			);

			const minimumNumberProducts = recentlyViewedHolder.hasAttribute(
				selectors$3.dataMinimum
			)
				? parseInt(recentlyViewedHolder.getAttribute(selectors$3.dataMinimum))
				: 4;
	...
	const cartSection = {
		onLoad() {
			...
			this.disabled =
				this.container.getAttribute(selectors$2.ajaxDisable) == "true";
			if (this.disabled) {
	...
	class TargetReferrer {
		...
		init() {
			if (
				this.locationPath.indexOf(
					this.el.getAttribute(selectors$1.dataTargetReferrer)
				) === -1 &&
				!window.Shopify.designMode
			) {
	...
	class PromoText {
		constructor(el) {
			...
			this.cookie = new PopupCookie(
				this.popup.getAttribute(selectors$1.cookieNameAttribute),
				"user_has_closed"
			);
	...
	class Look {
		...
		showProductsHolder() {
			...
			this.productsHolder.classList.toggle(
				classes.expand,
				this.currentDot.classList.contains(classes.active)
			);

			if (this.currentDot.classList.contains(classes.active)) {
				this.hasDefaultOpen = true;
				this.carousel.selectCell(
					Number(this.currentDot.getAttribute(selectors.dot)),
					true,
					selectCellAnimate
				);
			}
		...
		onBlockSelect(e) {
			if (this.slider && this.carousel) {
				const slide = this.container.querySelector(
					`[${selectors.blockId}="${e.detail.blockId}"]`
				);
				this.productsHolder.classList.add(classes.expand);
				this.carousel.selectCell(
					Number(slide.getAttribute(selectors.slideIndex)),
					true,
					true
				);
			}
